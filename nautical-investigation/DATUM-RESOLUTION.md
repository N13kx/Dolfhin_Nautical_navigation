...
**GDAL Z-sign convention:** Negative Z = below sounding datum; positive Z = above sounding datum (feature is exposed or barely covered at chart datum). A SOUNDG feature with Z = −4.2 means 4.2 m of navigable water depth below the sounding datum. A SOUNDG feature with Z = +0.8 means the feature sits 0.8 m above the sounding datum (shoal, bank, or area dry at chart datum). A DEPARE with DRVAL1 = −4.0 and DRVAL2 = −2.0 means water depth is between 2.0 m and 4.0 m below sounding datum. `VERIFIED_FROM_OFFICIAL_DOCUMENTATION` — *GDAL S-57 driver documentation (https://gdal.org/drivers/vector/s57.html); confirmed in REPORT.md §10 and §11.*

**Sample values observed:** Cell LI SOUNDG Z-values include both negative (−3.6 to −0.7 m, navigable depths) and positive (+0.5 to +0.8 m, features above datum). Cell RI SOUNDG Z-values are all positive (+0.9 to +10.0 m), indicating features or shoals that rise above the sounding datum — not navigable depths. `VERIFIED_FROM_FILE` — *REPORT.md §11 SOUNDG section.*
